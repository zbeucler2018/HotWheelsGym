# Borrowed from Moby's container name generator, thanks!
...
def get_random_name() -> str:
    """Returns a random name with int (0-5k) appended"""
    while True:
        name = f"{random.choice(tuple(LEFT))}_{random.choice(tuple(RIGHT))}"

        # Steve Wozniak is not boring, so we retry until a non-boring name is generated
        if name == "boring_wozniak":
            continue

        name += f"_{random.randint(0, 9999)}"
